################## PLOT GENE (SETS) EXPRESSION VALUE DISTRIBUTION FOR MICROARRAY GSE77791 BURN SHOCK ###########
...
### add new factor illness to meta file
  ...
  affyidrefs <-  c ("210948_s_at","201554_x_at","241981_at","202269_x_at","205660_at")
  
  keepnumbers <- numeric (); for (i in affyidrefs){thisone <- which (tabledfnr2$ID_REF == i); keepnumbers<- c (keepnumbers, thisone)}
  ...
  affyidrefs <- modules$modulegenesgpl570idrefs$GC_2
