################## PLOT GENE (SETS) EXPRESSION VALUE DISTRIBUTION FOR MICROARRAY GSE106878 SEPTIC SHOCK ###########
...
gse106878 <- readRDS ( "Tablegse106878")

gse106878meta <- readRDS ( "gse106878meta.rds")

modules <- readRDS ("modulesGC2andGC1.rds")
 
## EDIT META FILE

thetitles <- as.character (gse106878meta$title)
placebo <- grep ("placebo", thetitles)
hydrocortisone <- grep ("hydrocortisone", thetitles)
Pre <- grep ("Pre", thetitles)
Post<- grep ("Post", thetitles)
Preplacebo <- intersect (Pre, placebo)
Postplacebo <- intersect (Post, placebo)
Prehydrocortisone <- intersect (Pre, hydrocortisone)
Posthydrocortisone <- intersect (Post, hydrocortisone)
illness <- rep ("other", times = 94)
illness  [Preplacebo] <- "P0"
illness  [Postplacebo] <- "P24"
illness  [Prehydrocortisone] <- "GC0"
illness  [Posthydrocortisone] <- "GC24"
illnessfactor <- factor (illness, levels = c ("P0","GC0","P24","GC24"))
gse106878meta$illness <- illnessfactor


#### NAME META FILE MORE GENERALLY

metadfnr2 <- gse106878meta
samplesmeta2 <- as.character (metadfnr2$geo_accession)

#### NAME EXPRESSION TABLE MORE GENERALLY

tabledfnr2 <- gse106878
...
#### EDIT EXPRESSION TABLE (from log2 scale to linear scale)
...
summary (colMeans (tablenr2exp ,na.rm = TRUE))

tablesize <- dim (tabledfnr2)
samplenr <- tablesize [2] - 2

modulelist <- modules$modulegenesgpl10295idrefs
...
genes <- c ("LEF1","RAB27A","FAM20A","GBP1","OASL")
illuminaV2GPL10295idrefs <- c ("Qun3e4Pl7BEy6fdZX4", "KiQUC6h97dVnM9U_d4", "EnpItS.SAMZeiUiS2E", "Nk_t6ULcQ7VDNChBRU", "ukoevq3qjc6TUIS4R0")

keepnumbers <- numeric (); for (i in illuminaV2GPL10295idrefs){thisone <- which (tabledfnr2$ID_REF == i); keepnumbers<- c (keepnumbers, thisone)}
...
illuminaV2GPL10295idrefs <- modules$modulegenesgpl10295idrefs$GC_2
...
newfilename <- paste ("new", nextnr,"gse106878", ".pdf", sep = "")
