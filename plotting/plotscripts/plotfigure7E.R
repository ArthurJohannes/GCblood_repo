################## PLOT GENE (SETS) EXPRESSION VALUE DISTRIBUTION FOR MICROARRAY E-MTAB-7581 SEPTIC SHOCK ######
...
emtab7581 <- readRDS ("Tableemtab7581")

emtab7581meta <- readRDS ("emtab7581meta.rds" )
...
illness <- as.factor (as.character (emtab7581meta$Characteristics.srs.))
levels (illness) <- c ("SRS1","SRS2")
emtab7581meta2 <- emtab7581meta
emtab7581meta2$illness <- illness

#### NAME META FILE MORE GENERALLY

metadfnr2 <- emtab7581meta2

#### EDIT EXPRESSION TABLE (put sample order as in meta file)

samplesmeta2 <-as.character (emtab7581meta2$Source.Name)
samplestable <- colnames (emtab7581) [3:178]
sampleorder <- numeric (); for (i in samplesmeta2){thisone <- which (samplestable == i); sampleorder <- c (sampleorder, thisone)}
emtab7581nr2 <- emtab7581 [, c (1,2,sampleorder +2)]

nrofcolumns <- dim (emtab7581nr2 ) [2]

summary (colMeans (emtab7581nr2  [,3:nrofcolumns],na.rm = TRUE))
...
tablenr1exp <- apply (emtab7581nr2 [,3:nrofcolumns],2,funexp)

summary (colMeans (tablenr1exp ,na.rm = TRUE))

#### EDIT EXPRESSION TABLE (adjust column means)


thecolmeans <- colMeans (tablenr1exp, na.rm = TRUE)
midcolmean <- mean (thecolmeans)
divideby <- thecolmeans / midcolmean 

ttablenr1exp <- t (tablenr1exp)
ttablenr1expdiv  <- ttablenr1exp /divideby
tablenr2exp  <- t (ttablenr1expdiv )

summary (colMeans (tablenr2exp , na.rm = TRUE))


#### ADJUST AND NAME EXPRESSION TABLE MORE GENERALLY

tabledfnr2 <- cbind (emtab7581nr2 [,1:2],tablenr2exp)
...
newfilename <- paste ("new", nextnr,"emtab7581", ".pdf", sep = "")
