...
##  if expression table log2 scale
...
selectedgenes <- c ("MARCO","CDKN1C","C1QA","ALDH1A1","ASGR2","PPARG","MERTK","FLT3","ADAMTS2","VSIG4","GBP5","OASL","AZU1","MMP8","HP","MME","GYG1","ARG1","OLAH","DAAM2")

allgenesonplatform <- as.character (gse154918$IDENTIFIER)
keep <- numeric  (); for (i in selectedgenes){this <- which (allgenesonplatform == i); keep <- c (keep, this)}
...
##  if samples in expression table match samples in meta file, continue
...
for (i in 1:nrofgenes) {
  ...
  if (halfrange > 4.5) {maxpoint <- midpoint + halfrange; minpoint <- midpoint - halfrange; titlename <- paste (genesforplot [i], "!" , sep = " ")}
  ...
  p1 =  ggplot (df_forplot,  aes_string(y = log2values, x = "illness")) +
    ...
    geom_boxplot(width = 0.5,outlier.shape = NA,lwd = 1.5, fatten = 1.5, color = "green") +
    geom_dotplot(binaxis = "y", stackdir = "center", binwidth = 0.1 , alpha = 0.5, color = NA)+
    scale_y_continuous(limits = c (minpoint,maxpoint))+   
    ggtitle(titlename)+
    theme (axis.title.x = element_blank(), axis.title.y = element_blank(),axis.text.x = element_text(size =20))
  
  p2  =  ggplot (df_forplot,  aes_string(y = log2values, x = "illness")) +
    
    geom_boxplot(width = 0.5,outlier.shape = NA,lwd = 1.5, fatten = 3, color = "green") +
    geom_dotplot(binaxis = "y", stackdir = "center", binwidth = 0.1 , alpha = 0.5, color = NA)+
    scale_y_continuous(limits = c (minpoint,maxpoint))+   
    ggtitle(titlename)+
    theme (axis.title.x = element_blank(), axis.title.y = element_blank(),axis.text.x = element_text(size =40), axis.text.y = element_text(size =40),plot.title = element_text(size =60))
